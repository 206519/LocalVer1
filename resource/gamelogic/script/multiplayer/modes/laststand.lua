...
-- Human tags
	...
	return GetRandomItem(unitsToSpawn, function(t)
		...
		elseif unitCounts.BotInfantry >= 135 then -- maximum amount of infantry
			if UnitType("Infantry") and not UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		end

		if unitCounts.BotInfantry > 45 and unitCounts.BotATInfantry <= 10 then
			if UnitType("Infantry") and UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 4
			end
		elseif unitCounts.BotATInfantry >= 25 then
			if UnitType("Infantry") and UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		end
		...
		if UnitType("Squad") then
			if UnitType("Class1") then
				priorityMultiplier = priorityMultiplier * 5
			elseif UnitType("Class2") then
				priorityMultiplier = priorityMultiplier * 5 * 0.67
			else
				priorityMultiplier = priorityMultiplier * 3.5 * 0.25
			end
		end
