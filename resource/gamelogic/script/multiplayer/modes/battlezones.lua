...
local SpawnCooldownTime = {
	Min = 12 * 1000, -- 10 sec (ms)
	Max = 14 * 1000, -- 20 sec (ms)
}
...
-- Human tags
	...
	return GetRandomItem(unitsToSpawn, function(t)
		...
		if unitCounts.BotInfantry < 25 then -- minimum amount of infantry
			if UnitType("Infantry") then
				priorityMultiplier = priorityMultiplier
			elseif not UnitType("Doctrine") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		elseif unitCounts.BotInfantry >= 75 then -- maximum amount of infantry
			if UnitType("Infantry") and not UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		end

		if unitCounts.BotInfantry > 20 and unitCounts.BotATInfantry <= 2 then
			if UnitType("Infantry") and UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 3
			end
		elseif unitCounts.BotATInfantry >= 15 then
			if UnitType("Infantry") and UnitType("AT") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		end

		if unitCounts.BotInfantry + unitCounts.BotATInfantry >= 95 and unitCounts.BotATInfantry >= 35 then
			if UnitType("Infantry") then
				priorityMultiplier = priorityMultiplier * 0.1
			end
		end
		...
		if UnitType("Squad") then
			...
			elseif UnitType("Class2") then
				...
				priorityMultiplier = priorityMultiplier * 3 * 0.25
			end
		end
		...
		if UnitType("Cannon") then
			if UnitType("Class1") then
				priorityMultiplier = priorityMultiplier * 0.85
			elseif UnitType("Class2") then
				priorityMultiplier = priorityMultiplier * 0.85 * 0.67
			else
				priorityMultiplier = priorityMultiplier * 0.85 * 0.25
			end
		end
